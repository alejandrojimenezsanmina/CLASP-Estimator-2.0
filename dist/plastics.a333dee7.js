  ...
  fileReader.onload = function (e) {
    ...
    jsonData[1].forEach(function (row) {
      ...
      Object.keys(row).forEach(function (elem) {
        console.log("object keys is  ", elem);

        switch (elem) {
          case "Material":
            document.getElementById("materialSelected").selectedIndex = findValue(row[elem]);
            break;

          case "Units" && row[elem] === "in":
            console.log(row[elem]);
            document.querySelector("#inchRadio").checked = true;
            break;

          case "Units" && row[elem] === "mm":
            console.log(row[elem]);
            document.querySelector("#mmRadio").checked = true;
            break;

          case "Part Number":
            costForm.CPN.value = row["Part Number"];
            break;
      ...
      console.log("vale is ", document.querySelector("#materialSelected").selectedIndex);
    }); //  google.script.run
